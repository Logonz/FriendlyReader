...
class SynonymStore {
  constructor() {
    ...
    Mobx.reaction(
      () => this.data,
      data => console.log("SynonymStore: New data", this.data)
    );
  ...
  analyzeText(text, _callback) {
    ...
    if (!this.requesting) {
      ...
      if (this.lastText != text) {
        console.log("FR - SynStore -> New Data, Analyzing text");
        Database.API.SAPISRequest(
          text,
          function(data) {
            // Callback function
            ...
            if ("_synonym_suggestions" in data) {
              ...
              for (const word in suggestions) {
                for (const id in suggestions[word]) {
                  suggestions[word][id]["level"] = parseFloat(
                    suggestions[word][id]["level"]
                  );
                }
              }
        ...
        console.log(
          "FR - SynStore -> Text is the same as before, skipping call"
        );
  ...
  tagText(text, eventType, callback, cssClass) {
    ...
    if (typeof text === "string") {
      text = Database.utils.tokenizeText(text);
    }
